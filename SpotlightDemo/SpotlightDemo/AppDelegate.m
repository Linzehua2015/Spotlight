...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    for (NSInteger i = 0; i < 100; i++) {
        ...
        attributeSet.title = [NSString stringWithFormat:@"test-title %@", @(i).stringValue];
        attributeSet.contentDescription = @"test-description";
        UIImage *icon = [UIImage imageNamed:@"icon"];
        attributeSet.thumbnailData = UIImageJPEGRepresentation(icon, 1);
        
        NSString *uniqueId = [NSString stringWithFormat:@"demo_id %@", @(i).stringValue];
        CSSearchableItem *item = [[CSSearchableItem alloc] initWithUniqueIdentifier:uniqueId
